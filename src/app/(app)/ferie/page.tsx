...
function approvalMeta(row: LeaveRequestRow) {
  ...
  const who = personLabel({
    ...
    fallback: "Approvatore",
  });

  return `${who} · ${formatDateItalian(row.approved_at)}`;
}
...
export default async function FeriePage({ searchParams }: FeriePageProps) {
  ...
  const canDecide = profile.role === "addetto_turni" || profile.role === "amministratore";
              ...
              },
              {
                ...
                render: (row) => (
                  <div className="space-y-2">
                    <Badge variant={row.status === "in_attesa" ? "warning" : row.status === "approvato" ? "success" : "danger"}>
                      {leaveStatusLabelItalian(row.status)}
                    </Badge>
                    <p className="text-xs text-muted-foreground">Approvazione: {approvalMeta(row)}</p>
                  </div>
                ),
              },
              {
                header: "Note",
                render: (row) => <p className="whitespace-pre-wrap text-sm text-muted-foreground">{row.note?.trim() ? row.note : "—"}</p>,
              },
              {
                header: "Azioni",
                className: "min-w-[260px]",
                render: (row) => {
                  const isOwn = row.requester_profile_id === profile.id;
                  const isPending = row.status === "in_attesa";
