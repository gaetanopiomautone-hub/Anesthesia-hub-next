...
export async function saveShiftDraftAction(formData: FormData) {
  ...
  if (updateError) {
    redirect(turniPathWithContext(month, day, undefined, "Salvataggio bozza non riuscito. Riprova."));
  }
...
export async function submitShiftProposalAction(formData: FormData) {
  ...
  const { error: updateError } = await supabase.from("shifts").update(updatePayload).eq("id", parsed.data.shiftId);
  if (updateError) {
    redirect(turniPathWithContext(month, day, undefined, "Invio proposta non riuscito. Riprova."));
  }
  revalidatePath("/turni");
  revalidatePath("/turni-ferie");
...
export async function approveShiftAction(formData: FormData) {
  ...
  const { error: updateError } = await supabase.from("shifts").update(updatePayload).eq("id", parsed.data.shiftId);
  if (updateError) {
    redirect(turniPathWithContext(month, day, undefined, "Approvazione non riuscita. Riprova."));
  }
  revalidatePath("/turni");
  revalidatePath("/turni-ferie");
...
export async function rejectShiftAction(formData: FormData) {
  ...
  const { error: updateError } = await supabase.from("shifts").update(updatePayload).eq("id", parsed.data.shiftId);
  if (updateError) {
    redirect(turniPathWithContext(month, day, undefined, "Rifiuto proposta non riuscito. Riprova."));
  }
