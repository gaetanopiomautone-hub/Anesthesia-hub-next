-- Preferenza saluto / genere (solo scelta esplicita utente; null = formula neutra in dashboard).
...
begin
  if not exists (
    ...
    where t.typname = 'profile_gender'
...
comment on column public.profiles.gender is 'Preferenza saluto dashboard: male/female/other/prefer_not_to_say; null = neutro (Ciao …).';

-- Sostituisce RPC admin: parametro p_gender (null = non indicato in DB)
drop function if exists public.admin_apply_profile_update(
